...
const DiagnosisList = () => {
  ...
  return (
    <Container>
      ...
      <List>{state && state.dataList.most_likely.map((diag, idx) => <DiagnosisCard key={idx} diagnosis={diag} />)}</List>
      <DescriptionText style={{ marginTop: "2.4rem" }}>
        <span className="highlight">다음 질환도 의심되어요!</span>
      </DescriptionText>
      <List>{state && state.dataList.suspicious.map((diag, idx) => <DiagnosisCard key={idx} diagnosis={diag} />)}</List>
    </Container>
  );
};
